...
const Reports = () => {
  ...
  const [meta, setMeta] = useState<
    { page: number; limit: number; total: number; pages: number } | undefined
  >();
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {reports.map((r) => (
                  <Card key={r._id} className="border cursor-pointer">
                    <CardHeader>
                      <CardTitle
                        onClick={() => navigate(`/reports/${r._id}`)}
                        className="text-lg truncate hover:underline cursor-pointer "
                      >
                        {r.title}
                      </CardTitle>
                    </CardHeader>
            ...
            <div className="flex items-center justify-between mt-4">
              <div className="text-sm text-muted-foreground">
                صفحة {meta?.page || page} من {meta?.pages || 1} — إجمالي{" "}
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  variant="outline"
                  disabled={(meta?.page || page) >= (meta?.pages || 1) || loading}
        ...
        <AlertDialog open={!!toDelete} onOpenChange={(o) => !o && setToDelete(null)}>
          <AlertDialogContent>
            ...
            <div className="text-muted-foreground">
              سيتم حذف تقرير "{toDelete?.title}" ولا يمكن التراجع.
            </div>
            <AlertDialogFooter>
